...
void VehicleStateNode::onPublishTimer()
{
  ...
  if (!estimator_.valid) {
    if (allow_derived_ekf_health_) {
      ...
      //
      // Observed on ArduCopter V4.8.0-dev: /mavros/estimator_status receives nothing,
      // because MAVROS never requests the message (confirmed in sys_status.cpp) and every
      // runtime mechanism for asking -- set_stream_rate, set_message_interval, the legacy
      // SRx_EXTRA4 parameter -- either fails or no longer exists on that build. Without
      // this fallback ekf_healthy would be pinned false forever, which is a *false
      // negative*: it says nothing about the EKF, only that we cannot hear it. Downstream
      // that reads as an unhealthy vehicle, and j10_safety correctly zeroes every command,
      // so the whole autonomy chain is dead for want of a telemetry stream.
      //
      ...
      //
      // What this deliberately does NOT recover is the detail: accelerometer error flags
      // and the individual per-axis status bits are invisible here. That is why it is
      // opt-in and off by default -- a reported flag is strictly better evidence than an
      // inferred one, and this should be switched on only where the stream is known to be
      // absent.
      const auto age = pose_.age(stamp);
