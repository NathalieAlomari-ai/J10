...
| FCU log shows `EKF3 ... is using GPS` / `GPS 1: detected u-blox` despite `GPS_TYPE 0` in `sitl_indoor.parm`, or `PreArm: Motors: Check frame class and type` despite `ARMING_CHECK 0` | Stale `eeprom.bin` from an earlier SITL session in the same directory. ArduPilot's SITL persists parameters across runs in `eeprom.bin` (in the directory `sim_vehicle.py` was launched from — `~/ardupilot` here, since `run_sitl.sh` `cd`s there). A `--defaults`/`--add-param-file` value only seeds a parameter that has never been explicitly saved; once something is stored in `eeprom.bin`, the defaults file is silently ignored for that parameter on every later boot. Wipe it: `ros2 run j10_sim run_sitl.sh --console -- --wipe-eeprom` (the trailing `-- ...` passes through to `sim_vehicle.py`; `-w`/`--wipe-eeprom` is its flag for exactly this). Confirm with a clean run: no `GPS 1: probing`/`detected u-blox` lines at all, since `GPS_TYPE 0` disables the GPS backend entirely when applied correctly. |
| `vehicle_state_node` warns `/mavros/local_position/pose is 0.5x s old (limit 0.50 s)` repeatedly, but `/j10/vehicle/state` still shows real (non-zero, non-stamped-zero) pose data | Not the silent-topic-name bug (that fails with an all-zero, zero-timestamp pose — see above). This is ArduPilot's default `LOCAL_POSITION_NED` stream rate to a fresh MAVLink endpoint running close to 2 Hz with jitter, occasionally exceeding a tight timeout. `pose_timeout_sec` in `j10_mavlink/config/mavlink.yaml` is 1.0 s for this reason; if it still fires constantly, the stream rate is genuinely too low and worth requesting explicitly from ArduPilot (`SRx_POSITION`) rather than raising the timeout further. |
